...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      timeId: options.timeId, // "00001" 为测试数据，使用请改为options.timeId,
      movieId: options.movieId, // "00001" 为测试数据，使用请改为options.movieId,
    });
            ...
            for (var index1 in times[index].timesToday) {
              if (times[index].timesToday[index1].timeId === timeId) {
                that.setData({
                  ...
                  hallName: times[index].timesToday[index1].hallId + '号厅',
                  price: times[index].timesToday[index1].price,
                });
                console.log(that.data.hallName)
                wx.request({
                ...
                if (res.statusCode == 200) {
                  ...
                  if (res.data.error_code == "0") {
                    wx.showToast({
                      ...
                      success: function () {
                        wx.navigateTo({
                          url: '../completeorder/completeorder?totalPrice=' + totalPrice  //支付成功后前往的下一个页面
                        })
                      }
                    })
                  }
